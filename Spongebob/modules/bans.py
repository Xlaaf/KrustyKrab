...
from Spongebob.modules.helper_funcs.extraction import extract_user_and_text
from Spongebob.modules.helper_funcs.string_handling import extract_time
from Spongebob.modules.log_channel import gloggable, loggable
...
def punch(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id):
        message.reply_text("I really wish I could Cook this user....")
        return log_message
    ...
    if res:
        ...
        bot.sendMessage(
            chat.id,
            f"Cooking! {mention_html(member.user.id, html.escape(member.user.first_name))}.",
            parse_mode=ParseMode.HTML)
        log = (
            f"<b>{html.escape(chat.title)}:</b>\n"
            f"#KICKED\n"
...
__help__ = """
 • `/cookme`*:* cook the user who issued the command

*Admins only:*
 • `/ban <userhandle>`*:* bans a user. (via handle, or reply)
 • `/tban <userhandle> x(m/h/d)`*:* bans a user for `x` time. (via handle, or reply). `m` = `minutes`, `h` = `hours`, `d` = `days`.
 • `/unban <userhandle>`*:* unbans a user. (via handle, or reply)
 • `/cook <userhandle>`*:* cook a user out of the group, (via handle, or reply)
"""
